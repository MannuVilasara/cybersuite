...
const logger = createLogger({ service: 'auth-utils', level: LogLevel.INFO, pretty: true });
...
export const verifyToken = (token: string): JWTPayload | null => {
    try {
        ...
        logger.error('Token verification failed', { errorMessage: error instanceof Error ? error.message : 'Unknown error' });
        return null;
    }
};
