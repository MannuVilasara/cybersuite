...
const AuthorizeGitHubTab = () => {
  ...
  const checkGitHubConnection = async () => {
    try {
      const response = await axios.get(`${env.API_URL}/user/github/status`, {
  ...
  const handleGitHubAuth = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${env.API_URL}/user/github/auth`, {
        withCredentials: true,
      });
  ...
  const fetchRepositoriesFromSecureBot = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${env.SECUREBOT_URL}/user/${githubUser}/repositories`);
  ...
  const handleDisconnect = async () => {
    try {
      await axios.post(`${env.API_URL}/user/github/disconnect`, {}, { withCredentials: true });
      setIsConnected(false);
      setGithubUser(null);
      setSecureBotStatus(null);
      setRepositories([]);
              ...
              {isConnected ? (
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {checkingStatus || fetchingRepos ? 'Refreshing...' : 'Refresh'}
                  </button>
                  <button
        ...
        <div className="bg-black/40 backdrop-blur-xl rounded-2xl p-6 border border-white/10">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white">SecureBot Protection</h3>
                <p className="text-gray-400 text-sm">AI-powered repository security scanning</p>
              </div>
            </div>
          </div>
          ...
          {!isConnected ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-400 mb-4">Connect GitHub to enable SecureBot</p>
              <p className="text-gray-500 text-sm">
                SecureBot requires GitHub access to scan and protect your repositories
              </p>
            </div>
          ) : secureBotStatus ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
                <div className="flex items-center">
                  ...
                  <div>
                    <span className="font-medium text-green-400">SecureBot Installed</span>
                    <p className="text-gray-400 text-sm">GitHub App is active and monitoring</p>
                  </div>
                </div>
                <svg
              ...
              <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <h4 className="text-yellow-400 font-medium mb-2">SecureBot Features</h4>
                <ul className="space-y-1 text-gray-300 text-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-orange-500/10 border border-orange-500/30 rounded-lg">
                <div className="flex items-center">
                  ...
                  <div>
                    <span className="font-medium text-orange-400">SecureBot Not Installed</span>
                    <p className="text-gray-400 text-sm">Install GitHub App to enable protection</p>
                  </div>
                </div>
                <svg
